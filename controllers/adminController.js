...
const loadBanner = async (req,res)=>{
  try {
      ...
      try {
       console.log('hey i  am nithin')
        ...
        if(bannerData.is_active){
          bannerData = await Banner.findByIdAndUpdate({_id:id},{$set:{is_active:0}});
          state = 0
        }else{
          bannerData = await Banner.findByIdAndUpdate({ _id: id }, { $set: { is_active: 1 } })
          state=1
        }
        res.send({state})
        console.log(state)
    ...
    const loadOrders = async (req,res)=>{
      try {
        const orderData = await Order.find({}).populate('userId').sort({$natural:-1})
    ...
    const addOffer=async (req,res,next)=>{
      ...
      console.log(productData)
       ...
       await Product.findByIdAndUpdate({_id:productData[i]._id},{offer:offer})
     ...
     console.log(productData[i].name)
        ...
        const product = await Product.findByIdAndUpdate({_id:productData[i]._id},{
          price:price
          });
          console.log('end')
}
