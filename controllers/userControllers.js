...
const loadOTP = async(req,res,next)=>{
    try{
        ...
        if(existingUser){
            res.render('users/login',{message:'user already exits',user:req.session.user});
        }else{           
        ...
        res.render('users/otp',{user:req.session.user,mobile:req.session.tempUser.mobile,message:'otp sent successfully'})
        }
        ...
        const resendOtp =async (req,res,next)=>{
            try {
                ...
                newOtp=fastSms.sendMessage(req.session.tempUser.mobile,res);
                        ...
                        req.session.otp = newOtp
                        // const userData = await User.find();
                        res.render('users/otp',{user:req.session.user,mobile:req.session.tempUser.mobile,message:"otp resent successfully"})
        ...
        if(user){
            ...
            if(passwordMatch){
                if(user.is_verified){
                    ...
                    res.render('users/login', { message: "You have been temporarily blocked by the Administrator , Please login after sometime",user:req.session.user})
                }
            }else{
                res.render('users/login', { message: 'email or password is incorrect',user:req.session.user})
            }
        }else{
            res.render('users/login', { message: 'invalid user credentials', active: 0,user:req.session.user })
        }
     } catch (error) {
        res.redirect('/error')
        }
...
const loadProducts = async (req, res) => {
    try {
        ...
        if (sort == 0) {
            productData = await Product.find({ isAvailable: 1, $and: [{ category: arr }, { $or: [{ name: { $regex: '' + search + ".*" } }, { category: { $regex: ".*" + search + ".*" } }] }] }).sort({$natural:-1})
        ...
        res.render('users/products', { user: req.session.user, product: productData, category: categoryData, val: search, selected: category, order: sort, limit: limit,pageCount,page, })
        }
    } catch (error) {
        res.redirect('/error')
    }
...
const loadWishList = async (req,res,next)=>{
    try {
     if(req.session.user){
        ...
        res.render('users/wishList',{user:req.session.user,wishProduct:wishData.wishlist})
     }   else{
        res.redirect('/login')
     }
    } catch (error) {
...
const loadCart = async (req, res) => { 
    try {
        ...
        res.render('users/cart', { user: req.session.user, cartProduct:cartData.cart })}
        else{
            res.redirect('/')
        }
    } catch (error) {
...
const loadAddress = async (req, res) => {
    try {
        ...
        res.render('users/address', { user: req.session.user ,address:addressData })
    }else{
        res.redirect('login')
    }
    }catch (error) {
    ...
    //   console.log(req.ression.user.mobile)
      if(user.mobile == mobile){
        ...
        if (user) {
          ...
          res.render("users/resetOtp",{user:req.session.user,mobile:mobile,message:'otp sent successfully'});
        }else{
            console.log(3)
  ...
  const resendOtpVerification= async(req,res,next)=>{
    try {
                ...
                newOtp=fastSms.sendMessage(req.session.mob,res);
                        ...
                        console.log(newOtp)
                        // const userData = await User.find();
                        res.render('users/resetOtp',{user:req.session.user,mobile:req.session.mob,message:"otp resent successfully"})
    } catch (error) {
        res.redirect('/error')
    }
  }
...
const loadCheckOut = async(req,res)=>{
    try {
        if(req.session.user){
            ...
            res.render('users/userCheckOut',{ user: req.session.user ,products:cartData.cart,address:addressData,coupon })       
        }else{
            res.redirect('/')
        }
    } catch (error) {
...
const loadProfileAddress = async (req,res,next)=>{
    try {
        const address = await Address.find({ userId: req.session.user.id }).sort({$natural:-1})
...
const loadOrderDetails  = async function (req,res) {
    try { 
        if(req.session.user){
            const orderData = await Order.find({ userId: req.session.user_id }).sort({$natural:-1})
            res.render('users/orderDetails',{user: req.session.user, orders:orderData})
        }else{
            res.redirect('/login')
        }
  ...
  const singleOrderDetails = async (req,res)=>{
    try {
      ...
      res.render('users/singleOrderDetails',{user: req.session.user, order:orderData, head: 5, products:productData.products})
    } catch (error) {
        res.redirect('/error')
    }
  }  
  ...
  const cancellOrder= async (req,res,next)=>{
    try {
        ...
        if(req.session.user){
      const orderData =await Order.findByIdAndUpdate({_id:id},{
    ...
    try {
      ...
      if(user){
        ...
        console.log(1)
          ...
          res.render("users/resetotp2",{user:req.session.user,mobile:mobile,message:'otp sent successfully'});
        }else{
            console.log(3)
            res.redirect('/login')
        }
...
const resendOtpVerification2= async(req,res,next)=>{
    try {
                ...
                newOtp=fastSms.sendMessage(req.session.mob,res);
                        ...
                        console.log(newOtp)
                        // const userData = await User.find();
                        res.render('users/resetotp2',{user:req.session.user,mobile:req.session.mob,message:"otp resent successfully"})
    } catch (error) {
        res.redirect('/error')
    }
  }
...
const saveOnline2= async (req,res,next)=>{
    ...
    wallet:0
         ...
         await User.updateOne({_id:req.session.user},{$unset:{cart:1}})
         res.render('users/success2',{user:req.session.user})
